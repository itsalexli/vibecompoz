...
/**
 * @param abcText - your ABC notation
 * @returns MusicXML string
 */
export function convertAbcToMusicXml(abcText) {
  ...
  (function () {
    // all definitions inside an anonymous function
    "use strict";
    function repstr(n, s) {
    ...
    function dict(ks, vs) {
      // init object with key list and value list
      for (var i = 0, obj = {}; i < ks.length; ++i) obj[ks[i]] = vs[i];
      return obj;
    }
    function format(str, vals) {
      // help for sprintf string formatting
    ...
    function infof(str, vals) {
      abcOut.info(format(str, vals));
    }
    function endswith(str, suffix) {
      return str.indexOf(suffix, str.length - suffix.length) !== -1;
    }
    function keyints(obj) {
      return Object.keys(obj).map(function (x) {
        return parseInt(x);
      });
    }
    var max_int = Math.pow(2, 53); // Ecma largest positive mantissa.
    function sortitems(d, onkey) {
      // {key:value} -> [[key, value]] or [value] -> [[ix, value]]
      var xs = [],
        f,
        k;
      if (Array.isArray(d))
        for (k = 0; k < d.length; ++k) {
          if (k in d) xs.push([k, d[k]]);
        }
      ...
      if (onkey)
        ...
        }; // tie (1) -> reverse sort on key (0)
      xs.sort(f);
      return xs;
    }

    var note_ornamentation_map = {
      // for notations, modified from EasyABC
    ...
    var dynamics_map = {
      // for direction/direction-type/dynamics/
      p: "!p!",
      pp: "!pp!",
      ppp: "!ppp!",
      pppp: "!pppp!",
    ...
    Counter.prototype.clear = function (vnums) {
      // reset all counters
    ...
    Counter.prototype.prcnt = function (ip) {
      // print summary of all non zero counters
      for (var iv in this.counters["note"]) {
        if (this.getv("nopr", iv) != 0)
          infof("part %d, voice %d has %d skipped non printable notes", [
            ip,
            iv,
            this.getv("nopr", iv),
          ]);
        if (this.getv("nopt", iv) != 0)
          infof("part %d, voice %d has %d notes without pitch", [
            ip,
            iv,
            this.getv("nopt", iv),
          ]);
        if (this.getv("note", iv) == 0)
    ...
    Music.prototype.appendElemCv = function (voices, elem) {
      for (var v in voices) this.appendElem(v, elem); // insert element in all voices
    };
    Music.prototype.insertElem = function (v, elem) {
      // insert at the start of voice v in the current measure
    ...
    Music.prototype.appendElemT = function (v, elem, tijd) {
    ...
    Music.prototype.getLastMelis = function (voice, num) {
    ...
    Music.prototype.addChord = function (note, noot) {
    ...
    Music.prototype.addBar = function (lbrk, m) {
      // linebreak, measure data
      if (m.mdur && this.maxtime > m.mdur)
        infof("measure %d in part %d longer than metre", [
          m.ixm + 1,
          m.ixp + 1,
        ]);
      ...
      for (var v in this.vnums) {
        if (m.lline || m.lnum) {
          ...
          if (p) {
            // p == null: in measure 1 no previous measure is available
            var x = p.str; // p.str is the ABC barline string
            if (m.lline)
              // append begin of repeat, m.lline == ':'
              x = (x + m.lline).replace(/:\|:/g, "::").replace(/\|\|/g, "|");
            if (this.nvlt == 3) {
              // add volta number only to lowest voice in part 0
              if (
                m.ixp + parseInt(v) ==
                Math.min.apply(null, keyints(this.vnums))
              )
                x += m.lnum;
            } else if (this.nvlt == 4) {
              // add volta to lowest voice in each part
              if (parseInt(v) == Math.min.apply(null, keyints(this.vnums)))
                x += m.lnum;
            } else if (m.lnum) {
              // new behaviour with I:repbra 0
        ...
        if (m.attr)
          // insert signatures at front of buffer
    ...
    Music.prototype.outVoices = function (divs, ip) {
      ...
      for (iv in this.vnums) {
        ...
        var vn = [],
          vl = {}; // for voice iv: collect all notes to vn and all lyric lines to vl
        ...
        if (this.cpl > 0)
          this.bpl = 0; // option -n (max chars per line) overrules -b (max bars per line)
        else if (this.bpl == 0) this.cpl = 100; // the default: 100 chars per line

        var bn = 0; // count bars
        while (vn.length) {
          // while still measures available
          var ib = 1;
          var chunk = vn[0];
          while (ib < vn.length) {
    ...
    ABCoutput.prototype.mkHeader = function (
      stfmap,
      partlist,
      midimap,
      vmpdct,
      ...
      for (i = 0; i < partlist.length; ++i) {
        // collect partnames into accVce and staff groups into accStf
        x = partlist[i];
        try {
      ...
      for (i = 0; i < staffs.length && i < accVce.length; ++i) {
        ...
        snm = partabbrv
          ...
          .replace(/^:|:$/g, "");
        clfnms[firstVoice] =
          (nm ? 'nm="' + nm + '"' : "") + (snm ? ' snm="' + snm + '"' : "");
      }
      m = this.mnum > -1 ? "%%measurenb " + this.mnum + "\n" : "";
      hd = [format("X:%d\n%s\n%s", [this.X, this.title, m])];
      if (this.scale !== "") hd.push("%%scale " + this.scale + "\n");
      if (this.pagewidth !== "")
        hd.push("%%pagewidth " + this.pagewidth + "cm\n");
      if (this.leftmargin !== "")
        hd.push("%%leftmargin " + this.leftmargin + "cm\n");
      if (this.rightmargin !== "")
        hd.push("%%rightmargin " + this.rightmargin + "cm\n");
      ...
      for (vnum in this.clefs) {
        ...
        if (dmap.length && clef.indexOf("perc") < 0)
          clef = (clef + " map=perc").trim();
      ...
      if (kks.length) {
        // output SVG stuff needed for tablature
        ks = [];
        ...
        var tb = this.shiftStem
          ? tabSvg.map(function (x) {
              return x.replace("-3", "-6");
            })
          : tabSvg;
        kks.forEach(function (k) {
          ks.push(k.length > 1 ? format(k2, [k, k]) : format(k1, [k, k]));
        });
    ...
    function abcLyr(xs, melis) {
      ...
      for (var i = 0; i < xs.length; ++i) {
        ...
        } else if (endswith(x, "_") && !endswith(x, "\\_")) {
    ...
    function abcdur(nx, divs, uL) {
      // convert an musicXML duration d to abc units with L:1/uL
      ...
      den = t[1];
      if (nx.fact) {
        // apply tuplet time modification
        numfac = nx.fact[0];
        denfac = nx.fact[1];
      ...
      if (den > 64) {
        // limit the denominator to a maximum of 64
        var x = num / den,
          n = Math.floor(x);
        if (x - n < 0.1 * x) {
          num = n;
        ...
        infof("denominator too small: %d/%d rounded to %d/%d", [
          num,
          den,
          t[0],
          t[1],
    ...
    function abcMid(note) {
      ...
      p =
        60 +
        [0, 2, 4, 5, 7, 9, 11]["CDEFGAB".indexOf(nUp)] +
        (nUp != n ? 12 : 0);
    ...
    function staffStep(ptc, o, clef, tstep) {
      ...
      if (ndif) {
        // diatonic transposition == addition modulo 7
    ...
    function insTup(ix, notes, fact) {
      ...
      if (i > -1)
        // splice (i, 1) == remove 1 element at i
        nx.tup.splice(i, 1); // later do recursive calls when any starts remain
      var tix = ix; // index of first tuplet note
      fn = fact[0];
      fd = fact[1]; // xml time-mod of the higher level
      fnum = nx.fact[0];
      fden = nx.fact[1]; // xml time-mod of the current level
      ...
      while (ix < notes.length) {
        ...
        if (nx.tup.indexOf("start") > -1) {
          // more nested tuplets to start
          t = insTup(ix, notes, tupfact);
          ix = t[0];
          tupcntR = t[1]; // ix is on the stop note!
          tupcnt += tupcntR;
        } else if (nx.fact) {
    ...
    function mkBroken(vs) {
      // introduce broken rhythms (vs: one voice, one measure)
      vs = vs.filter(function (n) {
        return n instanceof Note;
      });
      var i = 0;
      while (i < vs.length - 1) {
        var n1 = vs[i],
          n2 = vs[i + 1]; // scan all adjacent pairs
        if (!n1.fact && !n2.fact && n1.dur > 0 && n2.beam) {
    ...
    function outVoice(measure, divs, im, ip, unitL) {
      // note/elem objects of one measure in one voice
      var ix = 0,
        t;
      while (ix < measure.length) {
        // set all (nested) tuplet annotations
        var nx = measure[ix];
      ...
      for (var i = 0; i < measure.length; ++i) {
        ...
        if (nx instanceof Note) {
          ...
          var cns = nx.ns.filter(function (n) {
            return endswith(n, "-");
          });
          cns = cns.map(function (n) {
            return n.slice(0, -1);
          }); // chop tie
          var tie = "";
          if (chord && cns.length == nx.ns.length) {
      ...
      while (vs.indexOf("!ped!!ped!") >= 0)
        vs = vs.replace(/!ped!!ped!/g, "!ped!");
      while (vs.indexOf("!ped-up!!ped-up!") >= 0)
        vs = vs.replace(/!ped-up!!ped-up!/g, "!ped-up!");
      while (vs.indexOf("!8va(!!8va)!") >= 0)
        vs = vs.replace(/!8va\(!!8va\)!/g, ""); // remove empty ottava's
      return vs;
    }

    function sortMeasure(voice, m) {
      voice.map(function (e, ix) {
        e.pos = ix;
      }); // prepare for stable sorting
      voice.sort(function (a, b) {
        return a.tijd - b.tijd || a.pos - b.pos;
      }); // (stable) sort objects on time
      var time = 0;
      var v = [];
      var rs = []; // holds rests in between notes
      for (var i = 0; i < voice.length; ++i) {
        // establish sequentiality
        var nx = voice[i];
        if (nx.tijd > time && chkbug(nx.tijd - time, m)) {
        ...
        if (nx.tijd < time) {
          ...
          if (o.tijd <= nx.tijd) {
            ...
            if (o.ns[0] == "z") {
              ...
              infof("overlap in part %d, measure %d: rest shortened", [
                m.ixp + 1,
                m.ixm + 1,
              ]);
            } else {
              // make a chord of overlap
    ...
    function getPartlist($ps) {
      // correct part-list (from buggy xml-software)
      function mkstop(num) {
        // make proper xml-element for missing part-group
    ...
    function parseParts(xs, d, e) {
      ...
      $x = xs.shift();
      if ($x[0].tagName == "part-group") {
        // mkstop -> element without context -> uppercase
        num = $x.attr("number");
        type = $x.attr("type");
        if (type == "start") {
          // go one level deeper
          ...
          // stop: close level and return group-data
          nums = e.pop(); // last open group number in stack order
          if (xs.length && xs[0].attr("type") == "stop")
            if (num != nums) {
              // two consequetive stops
              // in the wrong order (tempory solution)
              t = d[nums];
              d[nums] = d[num];
              d[num] = t; // exchange values    (only works for two stops!!!)
            }
        ...
        name = [
          "name_tuple",
          $x.find("part-name").text() || "",
          $x.find("part-abbreviation").text() || "",
        ];
        return [[name].concat(elems), rest];
      }
    }

    function bracePart(part) {
      ...
      if (part.length > 1) brace = ["{"].concat(brace).concat(["}"]);
      return brace;
    }

    function prgroupelem(x, gnm, bar, pmap, accVce, accStf) {
      // collect partnames (accVce) and %%score map (accStf)
      var y, nms;
      if (x[0] == "name_tuple") {
        // partname-tuple = ['name_tuple', part-name, part-abbrev]
        y = pmap.shift();
        if (gnm[0]) {
      ...
      } else if (x.length == 2 && x[0][0] == "name_tuple") {
    ...
    function prgrouplist(x, pbar, pmap, accVce, accStf) {
    ...
    function compUnitLength(iv, maten, divs) {
    ...
    function getMelisma(maat) {
      // get melisma from notes in maat
      var ms = [];
      for (var i = 0; i < maat.length; ++i) {
        // every note should get an underscore
        var note = maat[i];
    ...
    function perc2map(abcIn) {
      ...
      for (i = 0; i < ls.length; ++i) {
        ...
        if (x.indexOf("V:") >= 0 || x.indexOf("K:") >= 0) {
          ...
          if (x.indexOf("perc") >= 0 && x.indexOf("map=") == -1)
            x += " map=perc";
          if (x.indexOf("map=perc") >= 0 && maps[id].length > 0)
            abc.push("%%voicemap perc" + id);
    ...
    function Parser(options) {
      ...
      this.note_alts = [
        // 3 alternative notations of the same note for tablature mapping
    ...
    Parser.prototype.matchSlur = function (
      type2,
      n,
      v2,
      note2,
      ...
      if (n in this.slurBuf) {
        ...
        var type1 = t[0],
          ...
          grace1 = t[3];
        if (type2 != type1) {
          // slur complete, now check the voice
          if (v2 == v1) {
            // begins and ends in the same voice: keep it
            if (type1 == "start" && (!grace1 || !stopgrace)) {
    ...
    Parser.prototype.doNotations = function (note, $nttn, isTab) {
      ...
      if (trem.length) {
        ...
        if (type == "single") {
          ...
          if (this.tstep) {
            // jef's version
            if (type == "stop")
              note.before.unshift("!trem" + trem.text() + "!");
          } else {
            // my version
      ...
      if (!isTab)
        $fingering.each(function () {
          // handle multiple finger annotations
      ...
      for (var x of $wvln.toArray()) {
        switch ($(x).attr("type")) {
          case "start":
            note.before.unshift("!trill(!");
            break; // keep left-right order!
          case "stop":
            note.after += "!trill)!";
            break;
        }
      ...
      if (glis.length) {
        ...
        if (glis.attr("type") == "start")
          note.before.unshift(format("!%s(!", [lt])); // keep left-right order!
        else if (glis.attr("type") == "stop")
          note.before.unshift(format("!%s)!", [lt]));
      }
    };
    Parser.prototype.tabnote = function (alt, ptc, oct, v, ntrec) {
      ...
      for (i = 0; i < 4; ++i) {
        // support same note on 4 strings
        ...
        if (i == 3) {
          // new allocaion needed but none is free
          fmt =
            "rejected: voice %d note %s string %s fret %s remains: string %s fret %s";
    ...
    Parser.prototype.ntAbc = function (ptc, oct, $note, v, ntrec, isTab) {
      ...
      if (acc == "other") {
        ...
        if (theSMUFL == "accidentalLowerOneUndecimalQuartertone") {
          acc = "quarter-flat";
        } else if (theSMUFL == "accidentalRaiseOneUndecimalQuartertone") {
          acc = "quarter-sharp";
        } else if (theSMUFL == "accidentalDoubleFlatOneArrowDown") {
          acc = "flat-flat-down";
        } else if (theSMUFL == "accidentalFlatOneArrowDown") {
          acc = "flat-down";
        } else if (theSMUFL == "accidentalNaturalOneArrowDown") {
          acc = "natural-down";
        } else if (theSMUFL == "accidentalSharpOneArrowDown") {
          acc = "sharp-down";
        } else if (theSMUFL == "accidentalDoubleSharpOneArrowDown") {
          acc = "double-sharp-down";
        } else if (theSMUFL == "accidentalDoubleFlatOneArrowUp") {
          acc = "flat-flat-up";
        } else if (theSMUFL == "accidentalFlatOneArrowUp") {
          acc = "flat-up";
        } else if (theSMUFL == "accidentalNaturalOneArrowUp") {
          acc = "natural-up";
        } else if (theSMUFL == "accidentalSharpOneArrowUp") {
          acc = "sharp-up";
        } else if (theSMUFL == "accidentalDoubleSharpOneArrowUp") {
          acc = "double-sharp-up";
        } else {
          gotBadOther = true;
          ...
          if (!gotSMUFL) {
            if (
              theSMUFL.toLowerCase().indexOf("raise") != -1 ||
              theSMUFL.toLowerCase().indexOf("sharp") != -1
            ) {
              theSMUFL = "♯?";
            } else if (
              theSMUFL.toLowerCase().indexOf("lower") != -1 ||
              theSMUFL.toLowerCase().indexOf("flat") != -1
            ) {
              theSMUFL = "♭?";
            } else if (theSMUFL.toLowerCase().indexOf("natural") != -1) {
              theSMUFL = "♮?";
            } else {
              theSMUFL = "?";
            }
      ...
      if (ntrec.tab) return this.tabnote(alt, ptc, oct, v, ntrec);
      // implies s.tstep is true (options.t was given)
      else if (isTab && this.tstep) {
        var nt =
          ["__", "_", "", "^", "^^"][parseInt(alt || "0") + 2] +
          addoct(ptc, oct);
      ...
      if (acc === "" && alt === "") {
        ...
        // now see if we really must add an accidental
        alt = parseFloat(alt); // some xml files do have floats in <alter>
        if (p_v in this.curalts) {
        ...
        if (
          xs.some(function (x) {
            return $(x).attr("type") == "stop";
          })
        )
          return p; // don't alter tied notes
        infof("accidental %d added in part %d, measure %d, voice %d note %s", [
          alt,
          this.msr.ixp + 1,
          this.msr.ixm + 1,
          v + 1,
      ...
      if (gotBadOther) {
        ...
        if (alt == undefined) {
          ...
          if (!gotMatch) {
            if (
              acc.toLowerCase().indexOf("raise") != -1 ||
              acc.toLowerCase().indexOf("sharp") != -1
            ) {
              acc = "♯?";
            } else if (
              acc.toLowerCase().indexOf("lower") != -1 ||
              acc.toLowerCase().indexOf("flat") != -1
            ) {
              acc = "♭?";
            } else if (acc.toLowerCase().indexOf("natural") != -1) {
              acc = "♮?";
            } else {
              acc = "?";
            }
    ...
    Parser.prototype.doNote = function ($n) {
      ...
      var p =
        $n.find("pitch>step").text() ||
        $n.find("unpitched>display-step").text();
      var o =
        $n.find("pitch>octave").text() ||
        $n.find("unpitched>display-octave").text();
      ...
      if (note.grace && !this.ingrace) {
        // open a grace sequence
        this.ingrace = 1;
        note.before = ["{"];
      ...
      if (
        ...
        e.length &&
        e.text() == "none" &&
        (!isTab || v in this.hasStems || this.tstep)
      ) {
        note.before.push("s");
        abcOut.stemless = 1;
      }
      e = $n.find("accidental");
      if (e.length && e.attr("parentheses") == "yes")
        note.ntdec += "!courtesy!";
      var noot;
      ...
      if ($n.find("unpitched").length) {
        ...
        if (nh == "circle-x" || nh == "diamond" || nh == "triangle")
          noot = "_" + noot.replace(/\^/g, "").replace(/_/g, "");
      ...
      xs = $n
        ...
        .get();
      note.beam =
        xs.indexOf("continue") > -1 || xs.indexOf("end") > -1 || note.grace;
      xs = $n.find("lyric");
      var lyrlast = 0;
      for (var i = 0; i < xs.length; ++i) {
        var $e = $(xs[i]);
        var lyrnum = parseInt(
          ($e.attr("number") || "1").replace(/^.*verse/, "")
        );
    ...
    Parser.prototype.doAttr = function ($e) {
      ...
      if (fifths) {
        t = setKey(
          parseInt(fifths),
          $e.find("key>mode").first().text() || "major"
        );
        key = t[0];
        this.msralts = t[1];
        if (first && !steps && abcOut.key == "none") abcOut.key = key;
        // first measure -> header, if not transposing instrument and header not yet set.
          ...
          .each(function () {
            ...
            if (ty == "start") {
              ...
              voices.forEach(function (v) {
                dit.msc.insertElem(v, dit.repeat_str[n]);
              });
            } else if (ty == "stop") {
              ...
              dit.repeat_str[n][0] = format("[I:repeat %s%d]", [
                mid_str,
                repeat_count,
              ]);
      ...
      for (i = 0; i < clfs.length; i++) {
        ...
        if (stfdtl.length && (stfdtl.attr("number") || 1) == n) {
          ...
          var strings = stfdtl.find("staff-tuning"),
            tuning = [];
          strings.each(function () {
            tuning.push(
              $(this).find("tuning-step").text() +
                $(this).find("tuning-octave").text()
            );
          });
    ...
    Parser.prototype.findVoice = function (i, $es) {
      ...
      for (k = i + 1; k < $es.length; ++k) {
        // or to the voice of the next note
        $e = $es.eq(k);
        if ($e[0].tagName == "note") {
    ...
    Parser.prototype.doDirection = function ($e, i, $es) {
      ...
      function addDirection(dit, x, vs, tijd, stfnum) {
        ...
        vs.forEach(function (v) {
          if (tijd != null)
            // insert at time of encounter
            dit.msc.appendElemT(
              v,
              x.replace("(", ")").replace("ped", "ped-up"),
              tijd
            );
          else dit.msc.appendElem(v, x);
        });
      }
      function startStop(dit, dtype, vs, stfnum) {
        ...
        if (type in typmap) {
          ...
          x = typmap[type];
          if (k in dit.dirStk) {
            // closing direction already encountered
            sk = dit.dirStk[k];
            delete dit.dirStk[k];
            if (sk.type == "stop") addDirection(dit, x, vs, sk.tijd, stfnum);
            else {
              infof(
                "%s direction %s has no stop in part %d, measure %d, voice %d",
                [dtype, sk.type, dit.msr.ixp + 1, dit.msr.ixm + 1, vs + 1]
              );
          ...
          if (k in dit.dirStk) {
            // matching open direction found
            sk = dit.dirStk[k];
            delete dit.dirStk[k]; // into the same voice
            type = sk.type;
            vs = sk.vs; // same values as opening direction
            if (type == "stop") {
              infof(
                "%s direction %s has double stop in part %d, measure %d, voice %d",
                [dtype, type, dit.msr.ixp + 1, dit.msr.ixm + 1, vs + 1]
              );
              x = "";
            } else {
            ...
            // closing direction found before opening
      ...
      if (t.length) {
        ...
        if (minst.length) {
          ...
          for (v in this.vceInst) {
            // direction is for the indentified voice, not the staff
            if (this.vceInst[v] == minst.attr("id")) vs = v;
          }
          parm = prg ? "program" : "channel";
          inst = (prg ? prg - 1 : chn) + "";
          if (inst && abcOut.volpan > 0)
            this.msc.appendElem(vs, "[I:MIDI= " + parm + " " + inst + "]");
        }
      ...
      for (var ix = 0; ix < dirtyps.length; ++ix) {
        ...
        if (dirtyp.find("other-direction").text() == "diatonic fretting")
          this.diafret = 1;
      }
      if (tempo) {
        tempo = parseFloat(tempo).toFixed(0); // hope it is a number and insert in voice 1
        if (this.msc.tijd == 0 && this.msr.ixm == 0) {
          ...
          this.msc.appendElem(
            v1,
            format("[Q:%d/%d=%s]", [tempo_units[0], tempo_units[1], tempo])
          ); // otherwise -> 1st voice
      }
      if (wrdstxt) this.msc.appendElem(vs, '"' + plc + wrdstxt + '"', 1); // to first voice
    };
    Parser.prototype.doHarmony = function ($e, i, $es) {
      ...
      if (kind in kort) kind = kort[kind];
        ...
        // xml chord names: <triad name>-<modification>
        t = kind.split("-");
        triad = t[0];
        mod = t[1];
        kind = (accmap[triad] || "") + (modmap[mod] || "");
        if (kind.indexOf("sus") == 0) {
          sus = kind;
          kind = "";
        } // sus-suffix goes to the end
      ...
      for (i = 0; i < degrees.length; ++i) {
        ...
        $d = $(degrees[i]);
        kind +=
          (altmap[$d.find("degree-alter").text()] || "") +
          $d.find("degree-value").text();
      }
      kind = kind.replace("79", "9").replace("713", "13").replace("maj6", "6");
      bass =
        $e.find("bass>bass-step").text() +
        (altmap[$e.find("bass>bass-alter").text()] || "");

      // MAE 30 Jun 2024, inserted space before chord to avoid beaming issues
      this.msc.appendElem(
        vt,
        ' "' + root + alt + kind + sus + (bass && "/" + bass) + '"',
        1
      );
    };
    ...
    Parser.prototype.doPrint = function ($e) {
    ...
    Parser.prototype.doPartList = function ($e) {
    ...
    Parser.prototype.mkTitle = function ($e) {
      var title,
        ...
        ys,
        j;
      function filterCredits(y) {
        // y == filter level, higher filters less
        var cs = [],
          x,
          i;
        function _in(y, ys) {
          return y && ys.indexOf(y) > -1;
        } // y in ys
        function _fc(c) {
          return _in(c, x);
        } // c in (free variable) x
        for (i = 0; i < credits.length; ++i) {
          // skip redundant credit lines
          x = credits[i];
      ...
      for (i = 0; i < xs.length; ++i) {
        ...
        if (ctxt) {
          ...
          else if (ctype == "lyricist" || ctype == "transcriber")
      ...
      this.isSib =
        $e
          .find("identification>encoding>software")
          .text()
          .indexOf("Sibelius") >= 0;
    ...
    Parser.prototype.doDefaults = function ($e) {
      ...
      if (!abcOut.pagewidth && pagewidth)
        abcOut.pagewidth = pagewidth.toFixed(2);
      if (!abcOut.leftmargin && leftmargin != "")
        abcOut.leftmargin = (leftmargin * xmlScale).toFixed(2);
      if (!abcOut.rightmargin && rightmargin != "")
        abcOut.rightmargin = (rightmargin * xmlScale).toFixed(2);
    };
    Parser.prototype.locStaffMap = function ($part) {
      ...
      this.stfMap = {};
      this.clefMap = {}; // staff -> [voices], staff -> clef
      var ns = $part.find("measure>note");
      for (var i = 0; i < ns.length; i++) {
        // count staff allocations for all notes
        var $n = $(ns[i]);
        var v = parseInt($n.find("voice").text() || "1");
        if (this.isSib) v += 100 * ($n.find("staff").text() || "1"); // repair bug in Sibelius
        ...
        x = $n.find("stem");
        var noRest = $n.find("rest").length == 0;
        if (noRest && (x.length == 0 || x.text() != "none"))
          this.hasStems[v] = 1; // XML voice v has at least one stem
      }
      for (v in vmap) {
        // choose staff with most allocations for each voice
        var xs = [],
          vtup = vmap[v];
    ...
    Parser.prototype.addStaffMap = function (vvmap) {
      // vvmap: xml voice number -> global abc voice number
      ...
      for (j = 0; j < staff_keys.length; ++j) {
        // scan stfMap in alphabetical (xml) staff number order
        stf = staff_keys[j];
        voices = this.stfMap[stf]; // this.stfMap has xml staff and voice numbers
        (locmap = []), (nostem = []);
        for (i = 0; i < voices.length; ++i) {
          // voices is an array of xml voice numbers
        ...
        if (locmap.length) {
          ...
          for (i = 0; i < locmap.length; ++i) {
            ...
            if (clef.indexOf("tab") == 0) {
              if (nostem[i] && clef.indexOf("nostems") < 0)
                clef_attr = " nostems";
              if (this.diafret && clef.indexOf("diafret") < 0)
    ...
    Parser.prototype.addMidiMap = function (ip, vvmap) {
      // map abc voices to midi settings
      var instr = this.instMid[ip]; // get the midi settings for this part
      var defInstr,
        ks = Object.keys(instr);
      if (ks.length)
        defInstr = instr[ks[0]]; // default settings = first instrument
      ...
      var xs = [],
        ...
        i,
        ds,
        o = this; // problematic scope rules in javascript
      for (v in vvmap) {
        // xml voice num, abc voice num
        ks = Object.keys(this.drumNotes)
          .sort()
          .filter(function (x) {
            return x.split(";")[0] == v;
          }); // key == 'voice;abcnote'
        ds = ks.map(function (k) {
          ...
          return {
            nt: k.split(";")[1],
            step: o.drumNotes[k][0],
            midi: o.drumNotes[k][1],
            nhd: o.drumNotes[k][2],
          };
        });
        vabc = vvmap[v];
        ...
        if (id in instr)
          // id is defined as midi-instrument in part-list
          xs.push([vabc, instr[id].concat(ds)]);
        // get midi settings for id, append drummap
        else xs.push([vabc, defInstr.concat(ds)]); // only one instrument for this part
      }
      xs.sort(function (a, b) {
        return a[0] - b[0];
      }); // put abc voices in order
      xs.forEach(function (v_midi) {
        o.midiMap.push(v_midi[1]);
      });
      var snaarmap = ["E", "G", "B", "d", "f", "a", "c'", "e'"],
        v_noot,
        snaar,
        fret,
        noot;
      ...
      for (i = 0; i < ks.length; ++i) {
        ...
        xs.push(
          format("%%map tab%d %s print=%s heads=kop%s\n", [
            vabc,
            noot,
            snaarmap[snaar],
            fret,
          ...
          for (var i = 0; i < $es.length; i++) {
            ...
            switch (e.tagName) {
              ...
              case "attributes":
                ...
                break;
              case "direction":
                this.doDirection($e, i, $es);
                break;
              case "sound":
                this.doDirection($maat, i, $es);
                break; // sound element directly in measure!
              case "harmony":
                this.doHarmony($e, i, $es);
                break;
              case "barline":
                herhaal = this.doBarline($e);
                break;
              case "backup":
    ...
    vertaal = function (xmltree, options_parm) {
      // publish in the global name space
      ...
      var fnm = "",
        pad = "",
        X = 0; // fnm, pad are not used anywhere ...
      var options = {
        u: 0,
        b: 0,
        n: 0, // unfold repeats (1), bars per line, chars per line
        c: 0,
        v: 0,
        d: 0, // credit text filter level (0-6), no volta on higher voice numbers (1), denominator unit length (L:)
        m: 0,
        x: 0,
        t: 0, // no midi, minimal midi, all midi output (0,1,2), no line breaks (1), perc, tab staff -> voicemap (1)
        v1: 0,
        noped: 0, // all directions to first voice of staff (1), no pedal directions (1)
        stm: 0,
        mnum: -1, // translate stem elements (stem direction), number every mnum measures
        p: "f",
        s: 0, // page format: scale (1.0), width, left- and right margin in cm, shift note heads in tablature (1)
        addstavenum: 1,
      }; // Add measure numbers at the end of the staves
  ...
  const [musicXmlString] = vertaal(abcText, {});
